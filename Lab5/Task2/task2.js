...
// db.on and db.once inspired from http://blog.modulus.io/getting-started-with-mongoose
...
app.get('/', function(req, res){
    ...
    if(helper.isNewUser(cookies)){
        ...
        helper.filterEntries_mongo(data, Entry, function(err, results){
            if (err) return console.error(err);
            if(results.length > 0){
                helper.get_matching_entires_mongo(results[0], Entry, 3, function(err, matching_entries){
                    if (err) return console.error(err);
...
app.post('/post_coder', function(req, res){
    ...
    if(action=='next'){
        ...
        if(action=='create'){
            var data = helper.clean(req.session);
...
app.get("/get_coders", function(req, res){
	...
	var data = helper.clean(req.query);
